...
const topic = {
    ...
    appPath : mqttClient.parseTopic(
                    mqttClient.TOPIC.app_path,
                    {
                        serviceId: String(DATA.serviceId),
                        appId: String(DATA.appId)
                    }
                ),
    appNodes : mqttClient.parseTopic(
                    mqttClient.TOPIC.app_nodes,
                    {
                        serviceId: String(DATA.serviceId),
                        appId: String(DATA.appId)
                    }
                ),
    devices : `${mqttClient.TOPIC.devices}/guarana`,
    appDoc : mqttClient.parseTopic(
                    mqttClient.TOPIC.app_doc,
                    {
                        serviceId: String(DATA.serviceId),
                        appId: String(DATA.appId)
                    }
...
function beforeBack() {
    postFrameMessage({ type: 'publish', topic: '${topic.appPath}', message: '', retain: true });
    postFrameMessage({ type: 'publish', topic: '${topic.appNodes}', message: '', retain: true });
...
function getGuaranaDevices(d: string) {
    ...
    devs.forEach((d) => {
        NODES.push({
            id: `m360_RD${String(i).padStart(2, '0')}`,
            type: 'media',
            mimeType: 'application/x-ncl360',
            device: d
        });
        i++;
    });
}
mqttClient.addTopicHandler(topic.devices, getGuaranaDevices);


function publishAppData() {
    // Publish the current app
    mqttClient.publish(
        topic.currentApp,
        String(DATA.appId),
        true
    ...
    mqttClient.publish(
        topic.appPath,
        '/Users/joel/Coding/GingaDistrib/carnaval',
        true
    );
